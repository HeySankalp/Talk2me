...
// const sslOptions = {
//   key: fs.readFileSync('./certs/datopic.local-key.pem'),
//   cert: fs.readFileSync('./certs/datopic.local.pem')
// };
...
io.on("connection", (socket) => {
  // console.log("new user Connected...", socket.handshake.query, "|", socket?.id);
  ...
  // User Id Wise contact Get
  socket.on('userData', ({ userId, service }) => {
  ...
  socket.on('userClick', async ({ id, userId, receiverId, startm }) => {
    ...
    let cnt = await (await Msg.find({ $or: [{ sender_id: id }, { receiver_id: id }] })).length;
    console.log(cnt);
